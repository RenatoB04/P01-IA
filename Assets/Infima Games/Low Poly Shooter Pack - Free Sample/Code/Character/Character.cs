...
namespace InfimaGames.LowPolyShooterPack
{
    ...
    public sealed class Character : NetworkBehaviour
    {
        ...
        // --- INÍCIO DA CORREÇÃO (Campos de Visibilidade) ---
        [Header("Visibilidade (Ligar no Inspector)")]
        [Tooltip("O objeto dos teus braços (1ª pessoa, ex: SK_FP_CH_Default_Root)")]
        [SerializeField] private GameObject firstPersonModel;
        [Tooltip("O objeto do corpo (3ª pessoa, ex: BlueSoldier_Male)")]
        ...
        public override void OnNetworkSpawn()
        {
            ...
            if (owner)
            {
                // --- INÍCIO DA CORREÇÃO (VISIBILIDADE) ---
                // É O MEU JOGADOR
                if (firstPersonModel) firstPersonModel.SetActive(true);  // Mostra braços
                if (thirdPersonModel) thirdPersonModel.SetActive(false); // Esconde corpo
                                                                         // --- FIM DA CORREÇÃO ---
                ...
                {
                    // Esta lógica de spawn SÓ corre para o Dono (Host ou Cliente)
            ...
            else
            {
                // --- INÍCIO DA CORREÇÃO (VISIBILIDADE) ---
                // É UM JOGADOR REMOTO
                if (firstPersonModel) firstPersonModel.SetActive(false); // Esconde braços
                if (thirdPersonModel) thirdPersonModel.SetActive(true);  // Mostra corpo
                                                                         // --- FIM DA CORREÇÃO ---
        ...
        {
            // --- INÍCIO DA CORREÇÃO (Bug dos Braços) ---
            // Esta função é chamada no 'OnNetworkSpawn' (só para o owner)
            // e no 'Equip' (que também só corre no owner).
            if (!IsOwner) return;
            // --- FIM DA CORREÇÃO ---
